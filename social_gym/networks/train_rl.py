...
def load_model(algo, env):
    ...
    if algo == "TD3":
        ...
        model = TD3(policy, env, action_noise=action_noise, learning_rate=1e-4, buffer_size=int(1e6), learning_starts=training_starts_at, batch_size=256, 
	            ...
	            policy_delay=2, target_policy_noise=0.2, target_noise_clip=0.5, stats_window_size=100, tensorboard_log="./tensorboard_logs/", policy_kwargs=policy_kwargs, verbose=1, seed=42, device='cuda', _init_setup_model=True)
...
def load_train(log_dir_name, tensorboard_log_name, env, render=False):
    ...
    env.set_training_mode(areYouTraining=True, doYouRender=render, wantToUseRobots=True, allscenarios=False)
    ...
    saving_callback = SavingCallback(log_dir_name, save_freq=100000)
...
def load_test(log_dir_name, env, scenario, render=False, useRobot=True):
    ...
    if scenario == 0:
        #Run all scenarios - 20 runs of each
        ...
        #Run the scenario number based on provided value - 20 runs
...
def save_trajectories(episode, trajectories, useRobots, scenario):
    # Set the filename
    filename = f"/home/sagrawal/Desktop/SocialRobotForce/social_gym/networks/Traj_Logs/{scenario}_ep{episode}_{int(useRobots)}.txt"
...
def rl(env_name):
    model_name = "model_Ours_All_110724_v21_baseline"
    ...
    # load_test(log_dir_name, env, scenario=4, render=True, useRobot=True)
