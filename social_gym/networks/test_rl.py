...
def load_train(log_dir_name, tensorboard_log_name, env, render=False):
    ...
    env.set_training_mode(areYouTraining=True, doYouRender=render, wantToUseRobots=True, allscenarios=True)
...
def load_test(log_dir_name, env, scenario, render=False, useRobot=True, modelName='default'):
    ...
    if scenario == 0:
        ...
        for scene in range(1,6):
            ...
            for i in tqdm(range(eval_num)):
                for rob in [True,False]:
                    ...
                    save_trajectories(i, trajectories, useRobots, scene, modelName)
        ...
        for i in tqdm(range(eval_num)):
            ...
            avg_reward = avg_reward / eval_num
            useRobots, trajectories = env.get_pedestrian_trajectory()
            save_trajectories(i, trajectories, useRobots, scenario, modelName)
            #print('The agent collected %f reward on average during %d iterations' % (avg_reward, i))

def save_trajectories(episode, trajectories, useRobots, scenario, model_name):
    # Set the filename
    model_name = model_name
    log_dir_name = f"./Traj_Logs/{model_name}/"
    os.makedirs(log_dir_name, exist_ok=True)

    filename = log_dir_name + f"rl_{scenario}_ep{episode}_{int(useRobots)}.txt"
...
def rl(env_name):
    # model_name = "model_clean_attempt3_baseline2"
    model_name = "model_Ours_All_060924_v21_baseline"
    ...
    for val in [True]:
        ...
        load_test(log_dir_name, env, scenario=0, render=False, useRobot=val, modelName = model_name)

    # for i in range(1,6):
    #     load_test(log_dir_name, env, scenario=i, render=False, useRobot=False, modelName = "noRobot")

    # for i in range(1,6):
    #     load_test(log_dir_name, env, scenario=i, render=False, useRobot=True, modelName = model_name)
