...
#DWA code from - https://github.com/goktug97/DynamicWindowApproach/tree/master
            ...
            for i in tqdm(range(eval_num)):
                for rob in [True,False]:
                    ...
                    while not Done:
                        ...
                        pos, vel, goal, peds = env.get_dwa_arguments()
                        config = dwa.Config(
                            max_speed = 5.0,
                            min_speed = 0,
                        ...
                        vel = cartesian_to_polar_velocity(vel[0],vel[1],pos[2])
                        linear_vel, angular_vel = dwa.planning(pos, vel, goal, peds, config)
                        action = polar_to_cartesian_velocity(linear_vel, angular_vel, pos[2])
...
def save_trajectories(episode, trajectories, useRobots, scenario):
    # Set the filename
    filename = f"/home/sagrawal/Desktop/SocialRobotForce/social_gym/networks/Traj_Logs/dwa/dwarandomized_{scenario}_ep{episode}_{int(useRobots)}.txt"
